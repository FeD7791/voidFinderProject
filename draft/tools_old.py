...
def ctypes_input_params_builder(finder_type, **kwargs):
    if finder_type == "spherical":
        ...
        params = {
            # Double Values
            "RadIncrement": 0.0,  # cuanto incrementa la esfera a la hora de buscar con la random walk
            "BoxSize": 1000,
        ...
        # Load Input Params
        params1 = dict(
            list(params.items())[0:21]
        )  # Params that are double type
...
def preprocess_data_box(databox, **kwargs):
    ...
    df = df.round(
        kwargs["round_to"]
    )  # Rounding numbers of dataframe for more or less precision
    df.drop_duplicates(
...
def distance_void_tracer_classification(
    x_tracers, y_tracers, z_tracers, x_voids, y_voids, z_voids, rad_voids
):
    pos_void = np.array([x_voids, y_voids, z_voids])
    pos_void = np.transpose(pos_void)
    arr = []
    for i in range(len(x_tracers)):
        pos_box = np.array([[x_tracers[i], y_tracers[i], z_tracers[i]]])
...
def join_box_void(box, voids, **kwargs):
    ...
    if type(voids).__name__ in ["PopCornVoids", "ZobovVoids"]:
        ...
        for i in range(0, voids._void_len):
            n_index = n_index + list(
                i * np.ones(np.array(tracers_in_voids[i]).shape)
            )
        ...
        output_sparse_matrix = scipy.sparse.csr_matrix(
            (data, (row, col)), shape=(box._len, voids._void_len)
        )
...
def analysis_voids(
    box, voids, sparse, **kwargs
):  # n_items: the most relevant n_items

    params = {"n_items": len(voids)}
    for key, value in kwargs.items():
        params[key] = value
    ...
    if type(voids).__name__ == "PopCornVoids":
        df_filtered_param = df_voids.sort_values(
            by="void_volume", ascending=False
        )
    if type(voids).__name__ == "ZobovVoids":
    ...
    for i in df_filtered_index_cut:
        ...
        parameter_voids["velocity_norm"].append(
            np.linalg.norm(
                df_box.loc[tracer_indexes, ["vx", "vy", "vz"]].sum(axis=0)
            )
            / len(tracer_indexes)
        )
...
def void_size_function(box, voids, **kwargs):
    ...
    params = {"n_bins": 10, "save_fig": False, "plot": False}
    for key, value in kwargs.items():
        params[key] = value
...
def void_size_function_2(box, voids, **kwargs):
    ...
    # breaker br
    br = np.array(
        list(scl[1 : len(scl) - 1])
        + list(np.linspace(max(scl), mxlg, params["n_bins"]))
    )
...
def find_radius(box, voids, sparse):
    radius = []
    tracers = [
        [box.x.value[i], box.y.value[i], box.z.value[i]]
        for i in range(len(box))
    ]
    # DEPENDE DEL VOIDFINDER
    ##CONFIGURAR PARA VARIOS VOID FINDERS
    voids_centers = [
        [voids.x_void.value[i], voids.y_void.value[i], voids.z_void.value[i]]
        for i in len(voids)
    ]
    ##########################################################
    ...
    for i in range(sparse.shape[1]):
        ...
        radius.append(
            distances.sort(-1)[-1]
        )  # appending biggest distance to radius as candidate
    return radius

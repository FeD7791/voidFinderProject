...
def test_find_radius(mkbox, make_spherical_voids_params):
    ...
    sparse_matrix = np.zeros(
        (len(box), 2)
    )  # the matrix only have two ones in first column position 0 and 50
    sparse_matrix[0, 0] = 1
    ...
    max_distance = sp.spatial.distance.pdist(
        [box_0, box_50, void_center]
    ).sort(-1)
    assert (
        tools.find_radius(
            box=box, voids=spherical_voids, sparse=sparse_matrix
        )[0]
        == max_distance[-1]
    )
...
from voidfindertk.sphericalvf.spherical_tools import (
    SphericalVF,
    spherical_void_finder,
    SphericalVoids,
)
...
def test_sphericalvf_class(mkbox, make_spherical_voids_params):
    ...
    params = (
        make_spherical_voids_params()
    )  # Build params for spherical void output
    data_box = DataBox(box)  # Transform box to universal Box
    spherical_void = SphericalVoids(
        **params
    )  # Build artificial void output using params
    model_find_output = {
        "voids": spherical_void
    }  # Artificial output for mk_vbox step
    model = SphericalVF()
    output = model.mk_vbox(
        model_find_output, data_box
    )  # Build artificial mk_vbox
    assert box == data_box.box  # For spherical, data_box is the same as box
    assert data_box == model.preprocess(
        databox=data_box
    )  # preprocess in spherical won't modify data_box
...
# popcorn tools
import os
from voidfindertk.popcornvf.popcorn_tools import (
...
def test_popcorn_svf_input_data_builder(mkbox):
    ...
    )  # Path of the thest file
    path_vftk = os.path.split(
        path
    )  # path_vftk[0] path of the voidfinderproject folder
...
from voidfindertk.zobovvf.zobov_tools import (
    ZobovVoids,
    ZobovVF,
    calculate_tracers_inside_void,
)
from voidfindertk.models import DataBox
...
def test_calculate_tracers_inside_void(mkbox, make_zobov_voids_params):
    ...
    assert calculate_tracers_inside_void(box, voids) == [
        [1, 2, 3, 4, 5],
        [998, 997, 996],
    ]
...
def test_ZobovVF(mkbox, make_zobov_voids_params):
    ...
    box = mkbox(seed=42, size=1000)  # Make a box
    dbox = DataBox(box)
    params = make_zobov_voids_params(
        n_voids=100, CoreParticle=len(dbox.box)
    )  # Create params for zobov
    ...
    ### End model_find step
    # Test mk_vbox
    assert model.mk_vbox(voids, dbox).shape == (
        len(dbox.box),
        len(voids["voids"]),
    )
    # Test get_void_mass

...
dataset = (
    pathlib.Path("./datasets/") / "halos_ascii_1000_1024_npmin_10_z0.51.dat"
)
...
run_pocorn = pathlib.Path(
    "/home/jorgefederico/updates/vftk_actual002/voidFinderProject/run_popcorn"
)
sphfile = (
    run_pocorn / "tmplxhn4wcw2024-08-22T01:08:51.465883+00:00" / "sphfile.dat"
)

df = pd.read_csv(
    sphfile, delim_whitespace=True, names=["ID", "Rad", "x", "y", "z", "delta"]
)
...
for n, d in enumerate(dist):
    ...
    density_n_nat_d = (3 * n_nat[1:]) / (
        4 * np.pi * d[1:] ** 3
    )  ######Starts with index 0
    # Find all density values that are less than crit_density
    dens_values = np.where(density_n_nat_d < crit_density)[
        0
    ]  ##### Los indices quehay aqui empiezan con 0
    ...
    if len(dens_values) == 0:
        ...
        dist_max_index = np.where(
            d[dens_values + 1] == max(d[dens_values + 1])
        )[0][0]
...
density_n_nat_d = (3 * len(tracers_new[79530])) / (
    4 * np.pi * len(tracers_new[79530]) ** 3
)

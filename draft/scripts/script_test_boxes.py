...
centers = np.array(
    [np.array([10, 10, 10]), np.array([20, 20, 20]), np.array([30, 30, 30])]
)
...
kloud = build_spherical_void(
    delta=-0.7, centers=centers, radii=30, cloud=cloud
)
...
def build_spherical_void(
    ...
    ]  # +1 to ensure we remove more particles so density of the void < dens_voids
...
# Spherical voids
b_dist, b_index = grid.bubble_neighbors(
    centers[:2], distance_upper_bound=rad_max
)
...
b_dist, b_index = grid2.shell_neighbors(
    np.array([centers[2]]), distance_upper_bound=30, distance_lower_bound=5
)
...
dbox = box.Box(**{"x": x, "y": y, "z": z, "vx": x, "vy": y, "vz": z, "m": z})

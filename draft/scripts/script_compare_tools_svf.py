...
run_workdir_path = (
    workdir_path / "tmplxhn4wcw2024-08-22T01:08:51.465883+00:00"
)  # -0.7
...
model_find_parameters = {"run_work_dir": run_workdir_path, "box": dbox.box}
tinv, centers, extra = model.build_voids(model_find_parameters)

df = pd.read_csv(
    run_workdir_path / "sphfile.dat",
...
ax.axhline(
    y=average_density, label=f"{average_density:.4f}", c="green", ls="--"
)
ax.axhline(y=crit_density, label=f"{crit_density:.4f}", c="red", ls="--")
ax.set_xlabel("N tracers")
ax.set_ylabel("Density")
